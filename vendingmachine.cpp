//IM/2019/069 - Pasindu Prabhath 
#include <iostream>
using namespace std;

int productslist(int items[4][2]);
int calculations(int total, int coins[4], int balance, int cost);
int getcoins(int coins[4]);// 0 means Rs 10 coins
...
int main()
{
    ...
    while(continuining == 0)
    {
        ...
        cout<<"Enter the number of wanted product : "; //getting the product number
        cin>>inputnum;
...
int productslist(int items[4][2]) //prints products list
{
    for(int i = 0; i<4; i++)
    {
...
int getcoins(int coins[4]) // get coin inputs
{
...
int calculations(int total, int coins[4], int balance, int cost) // calculations
{
